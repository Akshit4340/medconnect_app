...
export async function registerUser(
  ...
  try {
    ...
    // Create the mandatory corresponding profile based on role
    if (input.role === 'patient') {
      await client.query(
        `INSERT INTO patients (tenant_id, user_id) VALUES ($1, $2)`,
        [tenantId, user.id],
      );
    } else if (input.role === 'doctor') {
      // For doctors, insert default values that they must change later in profile
      await client.query(
        `INSERT INTO doctors (tenant_id, user_id, specialisation, license_number, is_available) 
         VALUES ($1, $2, $3, $4, false)`,
        [
          tenantId,
          user.id,
          'General Practitioner',
          'PENDING-' + user.id.substring(0, 8),
        ],
      );
    }
  ...
  logger.info('User registered', { userId: user.id, tenantId: user.tenant_id });
